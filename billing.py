...
#---------------------- all functions ------------------------------
    ...
    # EDITED: Shared product query helper so every caller uses the same safe DB pattern.
    def _fetch_products(self,query,params=()):
        with closing(sqlite3.connect(database=r'ims.db')) as con:
            cur=con.cursor()
            cur.execute(query,params)
            return cur.fetchall()

    # EDITED: Single spot to clear and refill the product tree for reuse across flows.
    def _populate_product_table(self,rows):
        self.product_Table.delete(*self.product_Table.get_children())
        for row in rows:
            self.product_Table.insert('',END,values=row)

    # EDITED: Product list now loads through the helpers for less repetition.
    def show(self):
        try:
            rows=self._fetch_products("select pid,name,price,qty,status from product where status='Active'")
            self._populate_product_table(rows)
        except Exception as ex:
            messagebox.showerror("Error",f"Error due to : {str(ex)}")

    # EDITED: Name search is parameterized and reuses the helper for consistent UI updates.
    def search(self):
        try:
            search_txt=self.var_search.get().strip()
            if search_txt=="":
                messagebox.showerror("Error","Search input should be required",parent=self.root)
            else:
                rows=self._fetch_products("select pid,name,price,qty,status from product where name LIKE ?",(f"%{search_txt}%",))
                if rows:
                    self._populate_product_table(rows)
                else:
    ...
    # EDITED: Bail out when no row is selected to avoid index errors from the tree.
    ...
    # EDITED: Same guard here so the cart detail fields stay stable on empty clicks.
    ...
    # EDITED: Ensuring the bill folder exists and writing files via a context manager.
    def generate_bill(self):
        if self.var_cname.get()=="" or self.var_contact.get()=="":
            ...
            bill_path=os.path.join('bill',f'{str(self.invoice)}.txt')
...
'''
    ...
    # EDITED: Reworked stock updates to run inside one transaction and log totals cleanly.
    def bill_middle(self):
        try:
            with closing(sqlite3.connect(database=r'ims.db')) as con:
                cur=con.cursor()
                for pid,name,price,qty,stock in self.cart_list:
                    line_total=float(price)*int(qty)
                    self.txt_bill_area.insert(END,f"\n {name}\t\t\t{qty}\tRs.{line_total:.2f}")
                    remaining=int(stock)-int(qty)
