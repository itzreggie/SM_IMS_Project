...
- Rows with quantity at or below the shared low-stock threshold are highlighted so you can respond before shelves are empty.
...
### 9. low_stock.py
- This window highlights products whose on-hand quantity is at or below an alert threshold (default 5 units).
- It lists the low inventory items along with their quantity and status, and you can raise or lower the threshold to recalculate instantly.
- Saving the threshold persists it to the database, so the dashboard KPI and product list reuse the same alert level across sessions.
- Use it from the dashboard's new Low Stock button to proactively restock before items run out.
