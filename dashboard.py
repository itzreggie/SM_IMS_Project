...
# EDITED: Low stock module import keeps the dashboard aware of critical inventory levels.
from low_stock import get_low_stock_threshold, lowStockClass
...
class IMS:
    def __init__(self, root):
        self.root = root
        self.root.geometry("1350x820+110+80")
        ...
        # EDITED: Size the menu explicitly so the Exit button never slides below the viewport.
        self.root.update_idletasks()
        menu_height = max(450, self.root.winfo_height() - 200)
        LeftMenu = Frame(self.root, bd=2, relief=RIDGE, bg="white")
        LeftMenu.place(x=0, y=102, width=200, height=menu_height)
        ...
        # EDITED: Dedicated entry for the low stock monitor so it is one click away from the dashboard.
        ...
        # EDITED: Keep exit anchored to the bottom so new menu items never hide it.
        btn_exit = Button(
            LeftMenu, text="Exit",
            image=self.icon_side, compound=LEFT,
            padx=5, anchor="w",
        ...
        # EDITED: Dashboard card tracks products falling below the shared threshold.
        initial_threshold = get_low_stock_threshold()
        self.lbl_low_stock = Label(
            self.root, text=f"Low Stock (<= {initial_threshold})\n{{ 0 }}",
            bd=5, relief=RIDGE, bg="#560591",
            fg="white", font=("goudy old style", 20, "bold")
        )
    ...
    # EDITED: Quick launcher for the low stock monitor window.
    def low_stock(self):
        self.new_win = Toplevel(self.root)
    ...
    def update_content(self):
        try:
            # EDITED: Run each refresh inside a closing context so SQLite handles never leak.
            with closing(sqlite3.connect(database=os.path.join(BASE_DIR, 'ims.db'))) as con:
                cur = con.cursor()
                ...
                # EDITED: Count items at or below the threshold to keep the KPI accurate.
